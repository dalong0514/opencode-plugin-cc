...
- Prefer the helper over hand-rolled `git`, direct `opencode run` CLI strings, or any other Bash activity.
- Do not call `setup`, `models`, `review`, `status`, `result`, or `cancel` from `opencode:opencode-rescue`.
