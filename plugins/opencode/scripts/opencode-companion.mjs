...
function normalizeVariant(variant) {
  ...
  if (!/^[a-z0-9_-]+$/.test(normalized)) {
    throw new Error(
      `Unsupported reasoning effort "${variant}". Use a single variant token such as minimal, low, medium, high, max, or xhigh.`
    );
  }
  return normalized;
}
...
function buildSetupReport(cwd, actionsTaken = []) {
  ...
  const authStatus = opencodeStatus.available
    ? getOpencodeAuthStatus(cwd)
    : { loggedIn: false, detail: "opencode is not installed", models: [] };
  const config = getConfig(workspaceRoot);

  const nextSteps = [];
  if (!opencodeStatus.available) {
    nextSteps.push("Install opencode with `npm install -g opencode-ai` (or `bun add -g opencode-ai`).");
  }
  if (opencodeStatus.available && !authStatus.loggedIn) {
    nextSteps.push("Run `!opencode auth login` to configure a provider, then rerun `/opencode:setup`.");
  }
  if (authStatus.loggedIn) {
    nextSteps.push("Run `/opencode:models` to list models you can pass with `--model <provider/model>`.");
  }
  ...
  return {
    ready: nodeStatus.available && opencodeStatus.available && authStatus.loggedIn,
    node: nodeStatus,
...
function handleModels(argv) {
  ...
  const cwd = resolveCommandCwd(options);
  const report = listOpencodeModels(cwd);
  outputCommandResult(report, renderModelsReport(report), options.json);
}
...
function ensureOpencodeAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error(
      "opencode CLI is not installed. Install it with `npm install -g opencode-ai`, then rerun `/opencode:setup`."
    );
  }
}
...
function resolveLatestTrackedTaskSession(cwd, options = {}) {
  ...
  if (activeTask) {
    throw new Error(`Task ${activeTask.id} is still running. Use /opencode:status before continuing it.`);
  }
...
async function executeReviewRun(request) {
  ...
  const result = await runTurnWithSharedServer(context.repoRoot, {
    ...
    write: false,
    title: `opencode Companion Review: ${target.label}`,
  ...
  return {
    ...
    summary: firstMeaningfulLine(result.finalMessage, "Review finished."),
    jobTitle: "opencode Review",
    jobClass: "review",
...
async function executeTaskRun(request) {
  ...
  if (request.resumeLast) {
    const latestSession = resolveLatestTrackedTaskSession(workspaceRoot, {
...
function buildReviewJobMetadata(target) {
  return {
    kind: "review",
...
function buildTaskRunMetadata({ prompt, resumeLast = false }) {
  ...
  const title = resumeLast ? "opencode Resume" : "opencode Task";
...
async function handleReview(argv) {
  const { options, positionals } = parseCommandInput(argv, {
    valueOptions: ["base", "scope", "model", "effort", "variant", "cwd"],
  ...
  const metadata = buildReviewJobMetadata(target);
