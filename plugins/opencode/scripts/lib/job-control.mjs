...
function inferLegacyJobPhase(job, progressPreview = []) {
  ...
  for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
    ...
    if (line.startsWith("starting opencode") || line.startsWith("opencode session ready")) {
      return "starting";
    }
    if (line.startsWith("running tool:") || line.startsWith("starting collaboration tool:")) {
    ...
    if (line.startsWith("applying file change:")) {
      return "editing";
    }
    if (line.startsWith("turn completed")) {
      return "finalizing";
    }
    if (line.startsWith("opencode error:") || line.startsWith("failed:")) {
      return "failed";
    }
  }
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) {
    throw new Error(`No job found for "${reference}". Run /opencode:status to inspect known jobs.`);
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Job ${active.id} is still ${active.status}. Check /opencode:status and try again once it finishes.`);
  }

  if (reference) {
    throw new Error(`No finished job found for "${reference}". Run /opencode:status to inspect active jobs.`);
  }
