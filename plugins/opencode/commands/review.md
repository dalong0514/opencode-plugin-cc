...
- Do not weaken the review framing or rewrite the user's focus text.
- The companion script parses the other flags, but Claude Code's `Bash(..., run_in_background: true)` is what actually detaches the run.
- The review supports working-tree review, branch review, `--base <ref>`, extra focus text, and model selection via `--model <provider/model>` plus `--effort <level>`.
